...
import { switchMap, map, startWith, catchError, of, combineLatest } from 'rxjs';
...
export class AttributionTabComponent {
  ...
  sub = toObservable(this.inputs).pipe(
    switchMap(inp => {
      if (!inp) return of({ loading:false, error:null, period:null, daily:null });
      ...
      return combineLatest([period$, daily$]).pipe(
        ...
        startWith({ loading:true, error:null, period:null, daily:null }),
        catchError(e => of({ loading:false, error: e?.message ?? 'Failed', period:null, daily:null }))
      );
    })
