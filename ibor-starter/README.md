...
**What it does:** Calls `ibor-starter/data_etl.sh full` which runs three phases in sequence:
...
```bash
./ibor-starter/2_data_bootstrap.sh           # skip if already loaded
./ibor-starter/2_data_bootstrap.sh --force   # drop and reload everything
```
...
```bash
./ibor-starter/data_etl.sh full           # init + staging + curated in one shot
./ibor-starter/data_etl.sh init_infra     # schema only
./ibor-starter/data_etl.sh load_staging   # CSVs -> stg.* only
./ibor-starter/data_etl.sh load_main      # stg.* -> ibor.* only
```
...
| `Data not found (empty [])` | `./ibor-starter/2_data_bootstrap.sh --force` |
