...
from ai_gateway.routes.analyst_llm_router import create_analyst_llm_router
...
def create_app() -> FastAPI:
	...
	# Hybrid (deterministic hybrid REST)
	app.include_router(hybrid_router, tags=["hybrid"])

	# Deterministic Analyst REST (positions / trades / pnl proxies)
	app.include_router(make_analyst_router(analyst_service), prefix="", tags=["analyst"])

	# LLM-based Analyst chat (OpenAI Agent + tools)
	app.include_router(
		create_analyst_llm_router(llm_agent),
		tags=["analyst-llm"],
	)
